...
function tdotf_cat_tag_cloud( $args = '') {
  ...
  // grab already set tags
  
  $tag_query = tdotf_get_tdo_tag_query();
  $incoming_tags = false;
  if(!empty($tag_query) && $tag_query != false) {
    if(strpos($tag_query,',') != FALSE) {
        $incoming_tags = split(",",$tag_query);
    } else if(strpos($tag_query,' ') != FALSE) {
        $incoming_tags = split(" ",$tag_query);
    } else {
        $incoming_tags = array($tag_query);
    }
    $args['incoming_tags'] = $incoming_tags;
  }
  
  $key = 'tdotf_cat_tag_cloud_'.$args['cat'];
  $tags = wp_cache_get($key);
  // don't bother with cache if tag is set on category
  if($tags == false || $incoming_tags != false) {
    ...
    if($incoming_tags != false) {
        $my_query->set('tag_slug__and',$incoming_tags);
    }
  	...
  	foreach ( (array) $tags as $tag ) {
        ...
        if($args['cat'] == 0 || !isset($tdotf_tag_get_var)) {
            ...
            if(!empty($incoming_tags)) {
                $tags_var = implode("+",$incoming_tags);
                if(!in_array($tag->slug,$incoming_tags)){
                    $tags_var = $tag->slug . '+' . $tags_var;
                }
            }
            if(strpos($cat_link,"?") != FALSE) {
                $tag_links[$tag->name] = $cat_link."&".$tdotf_tag_get_var."=".$tags_var;
            } else {
                $tag_links[$tag->name] = $cat_link."?".$tdotf_tag_get_var."=".$tags_var;
            }
        }
